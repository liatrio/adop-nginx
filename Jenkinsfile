pipeline {
    ...
    stages {
            ...
            steps {
                ...
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-nginx:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-nginx",containerVersion)
                        sh "docker build -t liatrio/ldop-nginx:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-nginx:${env.BRANCH_NAME} liatrio/ldop-nginx:latest"
                sh "docker push liatrio/ldop-nginx:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-nginx:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
